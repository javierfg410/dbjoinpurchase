...
include_once('src/PurchaseJointHandler.php');
...
use Dbjointpurchase\src\PurchaseJointHandler;
...
class Dbjointpurchase extends Module
{
    ...
    /**
     * This method is a hook that is triggered when the product footer is displayed.
     * It fetches a list of products associated with a given product ID using the PurchaseJointHandler class,
     * generates a list of products if no associated products are found, prepares the products for the template,
     * and assigns them to Smarty for rendering in the template.
     *
     * @param array $params An associative array containing the product details.
     * @return string The HTML content to display in the product footer.
     */
    public function hookDisplayFooterProduct($params)
    {
        ...
        // Fetch the list of products associated with the given product ID using the PurchaseJointHandler class.
        $products_selected = PurchaseJointHandler::getJointsByProduct($id_product, true);
        // If no associated products are found, generate a list of products based on some default or fallback logic.
        if (!$products_selected) {
            $products_cat = $this->getProductsGenerate($id_product);
        } else {
            // If associated products are found, assign them to $products_cat.
    ...
    /**
     * This method is a hook that is triggered when the full width of the product is displayed.
     * It delegates the call to the hookDisplayFooterProduct method.
     *
     * @param array $params An associative array containing the product details.
     * @return string The HTML content to display in the product footer.
     */
    ...
    /**
     * This method is a hook that is triggered when the middle of the product is displayed.
     * It delegates the call to the hookDisplayFooterProduct method.
     *
     * @param array $params An associative array containing the product details.
     * @return string The HTML content to display in the product middle.
     */
    ...
    public
    function getTopSellerByCategory($id_product)
    {
        if ($this->premium == 1) {
    ...
    public
    function prepareBlocksProducts($products)
    {
        if ($products != false) {
...
// Unimos los arrays para evitar duplicados
        ...
        $this->context->controller->addJS($this->local_path . 'views/js/select.js');
        $this->context->controller->addCSS($this->local_path . 'views/css/select.css');
    }
